...
此版程式加入 fprintf 函式使用。
當程式運行過程中，若發生錯誤則可將錯誤訊息寫入 stderr。
...
### 編譯程式
```bash
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp gcc-5.1 gcc pwd_stderr.c -o pwd_stderr.o
```

### 執行程式
```bash
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp gcc-5.1 ./pwd_stderr.o -L
/usr/src/myapp

$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp gcc-5.1 ./pwd_stderr.o -P
/usr/src/myapp
