...
### 編譯程式
```bash
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp gcc-5.1 gcc -o pwd_err.o pwd_err.c
```

### 執行程式
```bash
$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp gcc-5.1 ./pwd_err.o -L
/usr/src/myapp

$ docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp gcc-5.1 ./pwd_err.o -P
/usr/src/myapp
